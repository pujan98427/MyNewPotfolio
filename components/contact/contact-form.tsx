...
export function ContactForm({turnstileSiteKey,idPrefix="contact-form",startedAt}:{turnstileSiteKey:string|null;idPrefix?:string;startedAt?:number}){
  ...
  useEffect(()=>{sessionStartedAtRef.current=startedAt??performance.now();let topicFrame=0;try{const raw=sessionStorage.getItem(DRAFT_KEY);if(!raw)return;const draft=JSON.parse(raw) as {email?:unknown;message?:unknown;topic?:unknown},form=formRef.current;if(!form)return;const email=typeof draft.email==="string"?draft.email.slice(0,254):"",message=typeof draft.message==="string"?draft.message.slice(0,3000):"",topic=typeof draft.topic==="string"&&draftTopics.has(draft.topic)?draft.topic as ContactTopic:"website",emailField=form.elements.namedItem("email"),messageField=form.elements.namedItem("message");if(emailField instanceof HTMLInputElement)emailField.value=email;if(messageField instanceof HTMLTextAreaElement)messageField.value=message;topicFrame=requestAnimationFrame(()=>setSelectedTopic(topic));}catch{sessionStorage.removeItem(DRAFT_KEY);}return()=>cancelAnimationFrame(topicFrame);},[startedAt]);
  ...
  return <form ref={formRef} className={styles.form} data-clarity-mask="true" data-state={submissionState} aria-busy={busy} onSubmit={submit} onInput={event=>updateDraft(event.currentTarget)}>
    ...
    <label className={styles.honeypot} aria-hidden="true">Website<input name="website" type="text" tabIndex={-1} autoComplete="off" aria-hidden="true" /></label>
    {turnstileSiteKey&&<TurnstileWidget siteKey={turnstileSiteKey} onToken={receiveTurnstileToken} resetKey={turnstileResetKey} />}
    <p className={`${styles.status} ${styles[submissionState]}`} role={submissionState==="error"?"alert":"status"} aria-live={submissionState==="error"?"assertive":"polite"} aria-atomic="true">{statusMessage}</p>
