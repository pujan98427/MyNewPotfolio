...
export function SvgBase64Tool(){
  ...
  const [advancedFormat,setAdvancedFormat]=useState<AdvancedFormat>("url-encoded");
  ...
  const imageTag=dataUri?`<img src="${dataUri}" alt="" />`:"";
  ...
  function reset(){setDirection("encode");setSvg("");setBase64("");setDecodeInput("");setError("");setCopied(null);setOutputTab("base64");setAdvancedFormat("url-encoded");setPreviewUri(null);setPreviewPending(false);setPreviewRemoved(0);setDragging(false);if(fileRef.current)fileRef.current.value="";}
  ...
  return <section className="svg-converter" aria-labelledby="svg-converter-heading">
    ...
    <aside className="svg-local-privacy" aria-label="SVG conversion privacy and access"><strong>Private by default</strong><div><p>Your SVG is processed locally in your browser and is not uploaded to our server.</p><p>No account, signup, email, API key or payment required. Paste an SVG and start immediately.</p></div></aside>
    <div className="svg-clear-row"><p>Keyboard friendly: use Ctrl/Cmd + V to paste, Ctrl/Cmd + A to select all, and Ctrl/Cmd + C to copy selected text.</p><button type="button" className="secondary" onClick={reset}><RefreshCw aria-hidden="true" /> Start over</button></div>
    ...
    <section className="svg-output" aria-labelledby="svg-output-heading"><div><p className="eyebrow">OUTPUT</p><h3 id="svg-output-heading">Copy the format you need.</h3></div>{base64?<div className="svg-output-list"><section className="svg-encoded-output" aria-label="Encoded SVG output">
      <div className="svg-output-tabs" role="tablist" aria-label="Choose SVG output format"><button id="svg-tab-base64" type="button" role="tab" aria-selected={outputTab==="base64"} aria-controls="svg-panel-base64" onClick={()=>setOutputTab("base64")}>Base64</button><button id="svg-tab-data-uri" type="button" role="tab" aria-selected={outputTab==="data-uri"} aria-controls="svg-panel-data-uri" onClick={()=>setOutputTab("data-uri")}>Data URI</button></div>
      <article id="svg-panel-base64" role="tabpanel" aria-labelledby="svg-tab-base64" hidden={outputTab!=="base64"}><header><strong>Raw Base64</strong><button type="button" aria-label="Copy raw SVG Base64" onClick={()=>void copy(base64,"base64")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="base64"?"Copied":"Copy Base64"}</span></button></header><pre><code>{base64}</code></pre></article>
      <article id="svg-panel-data-uri" role="tabpanel" aria-labelledby="svg-tab-data-uri" hidden={outputTab!=="data-uri"}><header><strong>Complete Base64 data URI</strong><button type="button" aria-label="Copy complete SVG data URI" onClick={()=>void copy(dataUri,"data-uri")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="data-uri"?"Copied":"Copy Data URI"}</span></button></header><pre><code>{dataUri}</code></pre></article>
      <article className="svg-img-tag-output"><header><strong>HTML image tag</strong><button type="button" aria-label="Copy SVG HTML image tag" onClick={()=>void copy(imageTag,"img-tag")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="img-tag"?"Copied":"Copy <img> tag"}</span></button></header><pre><code>{imageTag}</code></pre></article>
      <details className="svg-advanced-output"><summary>Advanced</summary><p>SVG can often be smaller as URL-encoded text than as Base64. Choose the format that best fits your project.</p><div className="svg-advanced-tabs" role="tablist" aria-label="Choose advanced SVG encoding"><button id="svg-advanced-base64" type="button" role="tab" aria-selected={advancedFormat==="base64"} aria-controls="svg-advanced-panel-base64" onClick={()=>setAdvancedFormat("base64")}>Base64</button><button id="svg-advanced-url" type="button" role="tab" aria-selected={advancedFormat==="url-encoded"} aria-controls="svg-advanced-panel-url" onClick={()=>setAdvancedFormat("url-encoded")}>URL-encoded SVG</button></div><article id="svg-advanced-panel-base64" role="tabpanel" aria-labelledby="svg-advanced-base64" hidden={advancedFormat!=="base64"}><header><strong>Base64 data URI</strong><button type="button" aria-label="Copy advanced Base64 SVG data URI" onClick={()=>void copy(dataUri,"data-uri")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="data-uri"?"Copied":"Copy Data URI"}</span></button></header><pre><code>{dataUri}</code></pre></article><div id="svg-advanced-panel-url" role="tabpanel" aria-labelledby="svg-advanced-url" hidden={advancedFormat!=="url-encoded"} className="svg-css-output"><article><header><strong>URL-encoded SVG data URI</strong><button type="button" aria-label="Copy URL-encoded SVG data URI" onClick={()=>void copy(utf8DataUri,"url-encoded")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="url-encoded"?"Copied":"Copy Data URI"}</span></button></header><pre><code>{utf8DataUri}</code></pre></article><article><header><strong>CSS background image</strong><button type="button" aria-label="Copy SVG CSS background-image declaration" onClick={()=>void copy(cssBackground,"css-background")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="css-background"?"Copied":"Copy CSS background"}</span></button></header><pre><code>{cssBackground}</code></pre></article><article><header><strong>Data URL with CSS url() function</strong><button type="button" aria-label="Copy SVG CSS URL function" onClick={()=>void copy(cssUrl,"css-url")}><Clipboard aria-hidden="true" /><span aria-live="polite">{copied==="css-url"?"Copied":"Copy CSS url()"}</span></button></header><pre><code>{cssUrl}</code></pre></article></div></details>
