...
export function RandomPickerTool(){
  ...
  return <div className={styles.workspace}><p className={styles.privacy}>Choices stay in this browser. Selection uses the browser’s cryptographic random source.</p><div className={styles.grid}><section className={styles.panel}><h2>Add your choices</h2><div className={styles.field}><label htmlFor="picker-options">One per line, or separated by commas</label><textarea id="picker-options" value={input} onChange={event=>{setInput(event.target.value);setWinner("")}} placeholder={"Alex\nMorgan\nSam"}/></div><details className={styles.advanced}><summary>More options</summary><label><input type="checkbox" checked={removeAfter} onChange={event=>setRemoveAfter(event.target.checked)}/> Remove the selected choice after each pick</label></details><div className={styles.actions}><button type="button" className={styles.button} onClick={pick}>Pick one</button><button type="button" className={styles.button} data-quiet onClick={()=>{setInput("");setWinner("");setMessage("")}}>Clear</button></div><p>{choices.length} {choices.length===1?"choice":"choices"}</p><p className={styles.status} data-error={Boolean(message)} role="status">{message}</p></section><section className={styles.panel}><h2>Selected</h2>{winner?<><div className={styles.resultHero}><span className={styles.resultLabel}>Selected</span><p className={styles.winner} aria-live="polite">{winner}</p><p>Chosen from {removeAfter?choices.length+1:choices.length} options.</p></div><div className={styles.actions}><button type="button" className={styles.button} onClick={()=>navigator.clipboard.writeText(winner)}>Copy result</button><button type="button" className={styles.button} data-quiet onClick={pick}>Pick again</button></div></>:<p>The selected choice will appear here.</p>}</section></div></div>;
}
