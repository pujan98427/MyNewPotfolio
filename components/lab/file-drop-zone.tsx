...
export function FileDropZone({accept,title,restrictions,actionLabel,multiple=false,disabled=false,onFiles}:FileDropZoneProps){
  ...
  return <label className={styles.zone} data-dragging={dragging||undefined} data-disabled={disabled||undefined} htmlFor={inputId} onDragEnter={event=>{event.preventDefault();if(!disabled)setDragging(true)}} onDragOver={event=>{event.preventDefault();event.dataTransfer.dropEffect=disabled?"none":"copy"}} onDragLeave={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node|null))setDragging(false)}} onDrop={event=>{event.preventDefault();setDragging(false);if(!disabled)deliver(event.dataTransfer.files)}}>
    ...
    <span><strong className={styles.title}>{dragging?"Drop files here":title}</strong><span className={styles.action}>{actionLabel??`Choose ${multiple?"files":"file"}`}</span><small className={styles.restrictions} id={helpId}>{restrictions}</small></span>
  </label>;
}
