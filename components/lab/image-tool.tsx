...
const labels:Record<ImageMode,string>={compress:"Compress",resize:"Resize image",convert:"Convert image",crop:"Crop image"};
...
<div className={styles.resultHero} role="status"><span className={styles.resultLabel}>{result.outcome==="original"?"Original kept":result.outcome==="tiny"?"Small saving":"Done"}</span>{mode==="crop"?<><strong className={styles.resultName}>{result.width} × {result.height}</strong><p>{prettyBytes(result.blob.size)}</p></>:<strong className={styles.resultName}>{result.outcome==="original"?"The original is already more efficient.":result.outcome==="tiny"?"This image is already quite small.":outputName}</strong>}{mode!=="resize"&&mode!=="crop"&&<><div className={styles.sizeChange}><span>{prettyBytes(file.size)}</span><b aria-hidden="true">↓</b><strong>{prettyBytes(result.blob.size)}</strong></div><p>{result.outcome==="original"?"No smaller version was created.":result.outcome==="tiny"?`The new version only saves ${savedPercent}%. Keeping the original may be better.`:`${savedPercent}% smaller`}</p></>}</div>
