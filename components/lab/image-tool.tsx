...
export function ImageTool({mode}:{mode:ImageMode}){
  ...
  const setSelectedImage=async(next:File)=>{setImageState("loading");setError("");if(source)URL.revokeObjectURL(source);if(result)URL.revokeObjectURL(result.url);setSource(null);setFile(null);setResult(null);try{const decoded=await decodeImageFile(next);setFile(next);trackProductEvent("file_selected",{tool_name:analyticsToolNames[mode]});setSource(decoded.url);setSourceWidth(decoded.width);setSourceHeight(decoded.height);setWidth(decoded.width);setHeight(decoded.height);setFormatComparisons([]);setFormat("image/webp");setImageState("ready")}catch{setImageState("error");setError("I couldn’t prepare this image. Try JPG, PNG or WebP instead.")}};
  ...
  const chooseFiles=async(files:File[])=>{
    ...
    setFile(null);setSource(null);setResult(null);setBatch(accepted);if(accepted.length)trackProductEvent("file_selected",{tool_name:analyticsToolNames[mode]});setError(rejected?`${rejected} ${rejected===1?"file was":"files were"} skipped because the format, content or size was not supported.`:files.length>10?"The first 10 images were added. Process another batch when these are finished.":"");
  };
  useEffect(()=>{
  ...
  const cropAnalyticsTimer=useRef<ReturnType<typeof setTimeout>|null>(null);
  useEffect(()=>()=>{if(cropAnalyticsTimer.current)clearTimeout(cropAnalyticsTimer.current)},[]);
  const recordCropResize=()=>{if(cropAnalyticsTimer.current)clearTimeout(cropAnalyticsTimer.current);cropAnalyticsTimer.current=setTimeout(()=>trackProductEvent("crop_resized",{tool_name:"image-cropper"}),400)};
  const resizeCrop=(clientX:number,clientY:number)=>{
    ...
    if(nextWidth!==cropWidthPercent||nextHeight!==cropHeightPercent)recordCropResize();const left=west?drag.anchorX-nextWidth:drag.anchorX,top=north?drag.anchorY-nextHeight:drag.anchorY;
  ...
  const resizeEdge=(clientX:number,clientY:number)=>{
    ...
    if(nextWidth!==cropWidthPercent||nextHeight!==cropHeightPercent)recordCropResize();setFreeCropWidth(nextWidth);setFreeCropHeight(nextHeight);
  ...
  const process=async()=>{
    ...
    try{
      ...
      trackProductEvent("tool_processed",{tool_name:analyticsToolNames[mode]});const formats={input_format:analyticsFormat(file.type),output_format:analyticsFormat(blob.type)};
  ...
  const download=()=>{if(!result||!file)return;trackProductEvent("result_downloaded",{tool_name:analyticsToolNames[mode]});trackProductEvent("image_downloaded",{tool_name:analyticsToolNames[mode],input_format:analyticsFormat(file.type),output_format:analyticsFormat(result.blob.type)});const anchor=document.createElement("a");anchor.href=result.url;anchor.download=outputName;anchor.click()};
  const continueWith=async(nextMode:ImageMode)=>{if(!result)return;trackProductEvent("image_handoff_clicked",{from_tool:analyticsToolNames[mode],to_tool:analyticsToolNames[nextMode],output_format:analyticsFormat(result.blob.type)});setHandoffBusy(nextMode);setError("");try{await saveImageHandoff(result.blob,outputName);trackProductEvent("tool_handoff_used",{from_tool:analyticsToolNames[mode],to_tool:analyticsToolNames[nextMode]});router.push(routes[nextMode])}catch{setError("This browser could not pass the image to the next tool. Download it instead.");setHandoffBusy(null)}};
