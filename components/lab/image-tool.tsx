...
<div className={styles.resultHero} role="status"><span className={styles.resultLabel}><span aria-hidden="true">✓ </span>{result.outcome==="original"?"Original kept":result.outcome==="tiny"?"Small saving":"Ready"}</span><strong className={styles.resultName}>{outputName}</strong><p>{result.width} × {result.height}<br/>{prettyBytes(result.blob.size)}</p>{mode==="compress"&&<p>{result.outcome==="original"?"No smaller version was created.":result.outcome==="tiny"?`The new version only saves ${savedPercent}%. Keeping the original may be better.`:`${savedPercent}% smaller`}</p>}</div>
