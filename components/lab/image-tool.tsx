...
export function ImageTool({mode}:{mode:ImageMode}){
  ...
  return <div className={`${styles.workspace} ${mode==="crop"?styles.cropTool:mode==="compress"?styles.compressorTool:""}`} data-tool-processing={busy||undefined} aria-busy={busy}>
    ...
    <div className={styles.grid}>
      <section className={styles.panel}>{mode!=="compress"&&<h2>Choose an image</h2>}
        <FileDropZone accept="image/png,image/jpeg,image/webp,image/gif,image/bmp,image/avif" title={uploadTitle} restrictions={`PNG, JPEG, WebP, GIF, BMP or AVIF · maximum 25 MB${mode==="compress"?" each · up to 10 images":""}`} multiple={mode==="compress"} disabled={busy} onFiles={files=>void chooseFiles(files)}/>
        {imageState==="error"&&<p className={styles.status} data-error role="status">{error}</p>}
...
{mode==="crop"&&<section className={styles.cropControls} aria-label="Crop controls"><fieldset className={styles.cropPresets}><legend>Crop shape</legend>{([['free','Free'],['1:1','1:1'],['16:9','16:9'],['4:5','4:5'],['9:16','9:16']] as const).map(([value,label])=><label key={value}><input type="radio" name="crop-shape" checked={ratio===value} onChange={()=>{setRatio(value);setCropX(50);setCropY(50)}}/> {label}</label>)}</fieldset><div className={styles.field}><label htmlFor="crop-zoom">Zoom · {cropZoom.toFixed(1)}×</label><input id="crop-zoom" type="range" min="1" max="3" step="0.1" value={cropZoom} onChange={event=>setCropZoom(Number(event.target.value))}/></div><div className={styles.actions}><button type="button" className={styles.button} data-quiet onClick={()=>setCropRotation(current=>(current+90)%360)} aria-label="Rotate image 90 degrees">Rotate</button><button type="button" className={styles.button} data-quiet onClick={resetCrop}>Reset</button></div><details className={styles.advanced}><summary>Fine tune</summary><div><fieldset className={styles.numericCrop}><legend>Crop position</legend><p>Use these fields instead of dragging, or move the focused crop area with the arrow keys. Hold Shift for larger moves.</p><div className={styles.row}><div className={styles.field}><label htmlFor="crop-position-x">Horizontal position (%)</label><input id="crop-position-x" type="number" min="0" max="100" step="1" value={Math.round(cropX)} onChange={event=>setCropX(Math.max(0,Math.min(100,Number(event.target.value)||0)))}/></div><div className={styles.field}><label htmlFor="crop-position-y">Vertical position (%)</label><input id="crop-position-y" type="number" min="0" max="100" step="1" value={Math.round(cropY)} onChange={event=>setCropY(Math.max(0,Math.min(100,Number(event.target.value)||0)))}/></div></div></fieldset>{ratio==="free"?<div className={styles.row}><div className={styles.field}><label htmlFor="crop-width">Selection width · {Math.round(freeCropWidth)}%</label><input id="crop-width" type="range" min="20" max="100" value={freeCropWidth} onChange={event=>setFreeCropWidth(Number(event.target.value))}/></div><div className={styles.field}><label htmlFor="crop-height">Selection height · {Math.round(freeCropHeight)}%</label><input id="crop-height" type="range" min="20" max="100" value={freeCropHeight} onChange={event=>setFreeCropHeight(Number(event.target.value))}/></div></div>:<div className={styles.field}><label htmlFor="crop-size">Selection size · {Math.round(cropScale)}%</label><input id="crop-size" type="range" min="25" max="100" value={cropScale} onChange={event=>setCropScale(Number(event.target.value))}/></div>}</div></details></section>}
          ...
          <div className={styles.actions}><button type="button" className={styles.button} onClick={process} disabled={busy}>{busy?"Working…":labels[mode]}</button><button type="button" className={styles.button} data-quiet onClick={clear} disabled={busy}>Clear</button></div><p className={styles.status} role="status" aria-live="polite">{busy?`${labels[mode]} in progress…`:""}</p></>}
      </section>
<section ref={resultRef} className={styles.panel} tabIndex={-1} aria-labelledby={`${mode}-result-heading`} style={{scrollMarginTop:"6rem"}} id={`${mode}-image-result`}><h2 id={`${mode}-result-heading`}>Result</h2>{result&&file?<>
<div className={styles.resultHero} role="status"><span className={styles.resultLabel}>{result.outcome==="original"?"Original kept":result.outcome==="tiny"?"Small saving":"Done"}</span>{mode==="convert"?<dl className={styles.originalSummary}><div><dt>{file.type.replace("image/","").toUpperCase()}</dt><dd>{prettyBytes(file.size)}</dd></div><div><dt>{result.blob.type==="image/webp"?"WebP":result.blob.type.replace("image/","").toUpperCase()}</dt><dd>{prettyBytes(result.blob.size)}</dd></div></dl>:mode==="crop"?<><strong className={styles.resultName}>{result.width} × {result.height}</strong><p>{prettyBytes(result.blob.size)}</p></>:<strong className={styles.resultName}>{result.outcome==="original"?"The original is already more efficient.":result.outcome==="tiny"?"This image is already quite small.":outputName}</strong>}{mode==="compress"&&<><div className={styles.sizeChange}><span>{prettyBytes(file.size)}</span><b aria-hidden="true">↓</b><strong>{prettyBytes(result.blob.size)}</strong></div><p>{result.outcome==="original"?"No smaller version was created.":result.outcome==="tiny"?`The new version only saves ${savedPercent}%. Keeping the original may be better.`:`${savedPercent}% smaller`}</p></>}</div>
        <div className={styles.preview}><img src={result.url} alt="Processed image preview" width={result.width} height={result.height}/></div>
    ...
    {mode==="compress"&&source&&result&&result.outcome!=="original"&&<section className={styles.comparison} aria-labelledby="compression-comparison"><h2 id="compression-comparison">Before and after</h2><p>Compare the complete images at the same fitted size. Neither preview is enlarged.</p><div><figure><figcaption>Original</figcaption><img src={source} alt="Original image before compression" width={width||result.width} height={height||result.height}/></figure><figure><figcaption>Compressed</figcaption><img src={result.url} alt="Image after compression" width={result.width} height={result.height}/></figure></div></section>}
  </div>;
}
