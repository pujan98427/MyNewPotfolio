...
export function ImageTool({mode}:{mode:ImageMode}){
  ...
  const setSelectedImage=async(next:File)=>{setImageState("loading");setError("");if(source)URL.revokeObjectURL(source);if(result)URL.revokeObjectURL(result.url);setSource(null);setFile(null);setResult(null);try{const decoded=await decodeImageFile(next);setFile(next);setSource(decoded.url);setSourceWidth(decoded.width);setSourceHeight(decoded.height);setWidth(decoded.width);setHeight(decoded.height);setFormatComparisons([]);setConvertIntent("website");setFormat("image/webp");setImageState("ready")}catch{setImageState("error");setError("I couldn’t prepare this image. Try JPG, PNG or WebP instead.")}};
  ...
  const uploadTitle=imageState==="error"?"Choose another image":mode==="compress"?"Drop images":"Drop an image";
