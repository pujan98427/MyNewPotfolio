  ...
  return <div className={styles.workspace} data-tool-processing={busy || undefined}>
    ...
    <div className={styles.grid}>
      ...
      <section ref={resultRef} className={styles.panel}>
        <h2>QR code</h2>
        {dataUrl ? <><div className={styles.resultHero} role="status"><span className={styles.resultLabel}>{scanWarning ? "Check colours" : "Ready"}</span><strong className={styles.resultName}>{scanWarning ? "Review this QR code." : "Your QR code is ready."}</strong><span className={styles.resultMetric}>{size} × {size} px</span><p>{scanWarning || (kind === "social" ? socialScanInstruction(socialService, socialPreset.label) : "Generated successfully with a clear quiet zone and practical contrast.")}</p></div><div className={styles.preview}><div className={styles.qrCard}><Image unoptimized className={styles.qr} src={dataUrl} width={size} height={size} alt={`Generated ${kindLabels[kind]} QR code`} />{caption && <p>{caption}</p>}</div></div><dl className={styles.stats}><div><dt>Type</dt><dd>{kindLabels[kind]}</dd></div>{kind === "social" && <div><dt>Link</dt><dd>{socialPreset.label}</dd></div>}{kind === "wifi" && <div><dt>Network</dt><dd>{wifiName}</dd></div>}<div><dt>Quiet zone</dt><dd>{margin} modules</dd></div><div><dt>Contrast</dt><dd>{contrastRatio(foreground, background).toFixed(1)}:1</dd></div></dl><div className={styles.encodedValue}><span>Encoded text</span><code>{encodedValue}</code><button type="button" className={styles.button} data-quiet onClick={() => void copyEncoded()}>{copied ? "Copied" : "Copy encoded text"}</button></div><div className={styles.downloadChoices}><div><button type="button" className={styles.button} aria-label="Download QR code only as PNG" disabled={Boolean(scanWarning)} onClick={() => { download(dataUrl, "qr-code.png"); trackProductEvent("qr_downloaded", { qr_type: kind, output_format: "png" }); }}>Download QR only</button><small>Good when you are adding it to your own design.</small></div><div><button type="button" className={styles.button} data-quiet disabled={Boolean(scanWarning) || !caption} onClick={downloadCard}>Download QR card</button><small>{caption ? "Includes your caption and is ready to print or share." : "Add a caption to make a ready-to-share QR card."}</small></div></div><div className={styles.actions}><button type="button" className={styles.button} data-quiet disabled={Boolean(scanWarning)} onClick={downloadSvg}>Download SVG</button><button type="button" className={styles.button} data-quiet disabled={Boolean(scanWarning)} onClick={() => window.print()}>Print</button></div>{scanWarning && <p className={styles.status} data-error="true" role="alert">{scanWarning}</p>}<p>Test the code with your phone before publishing it.</p></> : <p>Your QR code will appear here.</p>}
      </section>
    </div>
  </div>;
}
