...
export default async function ArticlePage({params}:Props){
  ...
  if(!article)notFound();
  return <main id="main"><article className="writing-detail"><Breadcrumbs items={[{label:"Writing",href:"/writing"},{label:article.title,href:`/writing/${article.slug}`}]} /><JsonLd data={articleStructuredData({headline:article.title,description:article.excerpt,path:`/writing/${article.slug}`,datePublished:article.date})} /><Link className="back-link" href="/writing"><ArrowLeft aria-hidden="true" /> All writing</Link><header><p className="eyebrow">{article.readingTime}</p><h1>{article.title}</h1><p>{article.excerpt}</p><time dateTime={article.date}>{new Intl.DateTimeFormat("en-GB",{dateStyle:"long"}).format(new Date(article.date))}</time></header><div className="article-body">{article.sections.map(section=><section key={section.heading}><h2>{section.heading}</h2><p>{section.body}</p></section>)}</div></article></main>;
}
