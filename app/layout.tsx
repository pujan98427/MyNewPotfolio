...
import { ContactChatBoundary } from "@/components/contact/contact-chat-boundary";
import { turnstileSiteKeyForClient } from "@/lib/contact/turnstile";
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { const turnstileSiteKey=turnstileSiteKeyForClient();return <html lang="en" className={`${sans.variable} ${serif.variable}`}><body><a className="skip-link" href="#main">Skip to content</a><SiteNavigation />{children}<SiteFooter /><ContactChatBoundary turnstileSiteKey={turnstileSiteKey} /></body></html>; }
