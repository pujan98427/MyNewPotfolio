...
import { isClarityConsentRequired, normalizeClarityProjectId } from "@/lib/analytics/clarity-config";
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { const turnstileSiteKey=turnstileSiteKeyForClient();const clarityProjectId=normalizeClarityProjectId(process.env.NEXT_PUBLIC_MICROSOFT_CLARITY_PROJECT_ID);const consentSetting=process.env.NEXT_PUBLIC_MICROSOFT_CLARITY_REQUIRE_CONSENT??process.env.NEXT_PUBLIC_MICROSOFT_CLARITY_ENABLED;const clarityRequireConsent=isClarityConsentRequired(consentSetting);return <html lang="en" className={`${sans.variable} ${serif.variable}`}><body><a className="skip-link" href="#main">Skip to content</a><SiteNavigation />{children}<SiteFooter /><ContactChatBoundary turnstileSiteKey={turnstileSiteKey} />{clarityProjectId&&<ClarityConsent projectId={clarityProjectId} requireConsent={clarityRequireConsent} />}</body></html>; }
