...
import { logContactFailure,logContactRejection } from "@/lib/contact/diagnostics";
...
export async function POST(request:NextRequest){
  ...
  if(origin&&origin!==SITE_URL&&origin!==request.nextUrl.origin){logContactRejection("origin");return failure("REQUEST_REJECTED","This request could not be accepted.",403);}
  if(!browserRequestOriginIsAllowed(request)){logContactRejection("origin");return failure("REQUEST_REJECTED","This request could not be accepted.",403);}
  const contentType=request.headers.get("content-type")??"";
  if(!contentType.toLowerCase().startsWith("application/json")){logContactRejection("validation");return failure("UNSUPPORTED_MEDIA_TYPE","Send the contact form as JSON.",415);}
  const declaredSize=Number(request.headers.get("content-length")??0);
  if(declaredSize>CONTACT_LIMITS.body){logContactRejection("validation");return failure("PAYLOAD_TOO_LARGE","That message is too large.",413);}
  let input:unknown;try{const raw=await request.text();if(new TextEncoder().encode(raw).byteLength>CONTACT_LIMITS.body){logContactRejection("validation");return failure("PAYLOAD_TOO_LARGE","That message is too large.",413);}input=JSON.parse(raw);}catch{logContactRejection("validation");return failure("INVALID_REQUEST","The message could not be read.",400);}
  let contact;try{contact=validateContactInput(input);}catch{logContactRejection("validation");return failure("VALIDATION_ERROR","Check your email and message.",400);}
  ...
  if(!turnstile.ok){
    logContactRejection("turnstile",contact.requestId);
  ...
  try{if(!contactDeliveryIsEnabled()){logContactRejection("configuration",contact.requestId);return failure("DELIVERY_DISABLED","Contact delivery is disabled in this environment.",503);}await sendContactEmail(contact);return json({ok:true});}
