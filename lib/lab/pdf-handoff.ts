...
export async function clearPdfHandoff():Promise<void>{const database=await openDatabase();try{await new Promise<void>((resolve,reject)=>{const transaction=database.transaction(STORE,"readwrite");transaction.objectStore(STORE).delete(RECORD_KEY);transaction.oncomplete=()=>resolve();transaction.onerror=()=>reject(new Error("Temporary PDF storage could not be cleared."));})}finally{database.close()}}
