...
const staticCards:Record<string,OpenGraphCard>={
  ...
  "/writing":{title:"Behind the tools.",eyebrow:"Notes from the Lab",description:"Practical documentation for the free tools in Pujan’s Lab."},
...
export function resolveOpenGraphCard(path:string):OpenGraphCard|undefined{
  ...
  if(path.startsWith("/writing/")){const item=articles.find(article=>`/writing/${article.slug}`===path);return item?{title:item.title,eyebrow:"Notes from the Lab",description:item.excerpt}:undefined;}
}
