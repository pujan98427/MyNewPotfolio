...
export function creativeWorkStructuredData(input:{name:string;description:string;path:string;image:string;externalUrl:string;year?:string}):JsonLdNode{return {"@context":"https://schema.org","@type":"CreativeWork",name:input.name,description:input.description,url:absoluteCanonicalUrl(input.path),image:absoluteCanonicalUrl(input.image),dateCreated:input.year,contributor:personReference,sameAs:input.externalUrl};}
